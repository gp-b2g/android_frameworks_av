...
bool MPEG4Writer::exceedsFileSizeLimit() {
    ...
    // If the moov box exceeds the reserved space, we need to append it to the
    // end and the reserved space will be considered wasted.
...
void MPEG4Writer::Track::updateTrackSizeEstimate() {
     ...
     *                   stco or co64
     */
    static const int64_t kBaseUrlBoxSize = kBaseMpegBoxSize + 4;
    static const int64_t kBaseStcoBoxSize = kBaseMpegBoxSize + 8; // aka CO64
    static const int64_t kEntryStcoBoxSize = 4;
    static const int64_t kEntryCo64BoxSize = 8;
    static const int64_t kBaseStszBoxSize = kBaseMpegBoxSize + 12;
    static const int64_t kEntryStszBoxSize = 4;
    static const int64_t kBaseStscBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kEntryStscBoxSize = 12;
    static const int64_t kBaseSttsBoxSize = kBaseMpegBoxSize + 16;
    static const int64_t kEntrySttsBoxSize = 8;
    static const int64_t kBaseStsdBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kBaseDrefBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kBaseDinfBoxSize = kBaseMpegBoxSize;
    static const int64_t kBaseHdlrBoxSize = kBaseMpegBoxSize + 36;
    ...
    static const int64_t kBaseTkhdBoxSize = kBaseMpegBoxSize + 96;
    static const int64_t kBaseTrakBoxSize = kBaseMpegBoxSize;

    // audio boxes
    static const int64_t kBaseSmhdBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kBaseEsdsABoxSize = kBaseMpegBoxSize + 29;
    static const int64_t kBaseDamrBoxSize = kBaseMpegBoxSize + 9;
    static const int64_t kBaseStsdABoxSize = kBaseMpegBoxSize + 28;

    // video boxes
    static const int64_t kBaseVmhdBoxSize = kBaseMpegBoxSize + 12;
    static const int64_t kBaseEsdsVBoxSize = kBaseMpegBoxSize + 29;
    static const int64_t kBaseD263BoxSize = kBaseMpegBoxSize + 7;
    static const int64_t kBaseAvccBoxSize = kBaseMpegBoxSize;
    static const int64_t kBaseStsdVBoxSize = kBaseMpegBoxSize + 78;
    static const int64_t kBasePaspBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kBaseStssBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kEntryStssBoxSize = 4;
    static const int64_t kBaseCttsBoxSize = kBaseMpegBoxSize + 8;
    static const int64_t kEntryCttsBoxSize = 8;

    // Calculate fixed minimum header for a track (above for file structure).
    mEstimatedTrackSizeBytes = kBaseTrakBoxSize +
                               kBaseTkhdBoxSize +
                               kBaseMdiaBoxSize +
                               kBaseMdhdBoxSize +
                               kBaseHdlrBoxSize +
    ...
    // We add 1 to all of the counts because if we accept the sample, it may
    // increase the size of the file beyond what was requested due to a longer
    // trak box.
    mEstimatedTrackSizeBytes +=
        (mStscTableEntries->count() + 1) * kEntryStscBoxSize +
        (mSttsTableEntries->count() + 1) * kEntrySttsBoxSize +
        (mOwner->use32BitFileOffset()
            ? (mStcoTableEntries->count() + 1) * kEntryStcoBoxSize
            : (mCo64TableEntries->count() + 1) * kEntryCo64BoxSize) +
        (mSamplesHaveSameSize
            ? kEntryStszBoxSize
            : (mStszTableEntries->count() + 1) * kEntryStszBoxSize);

    if (!mEstimatedCodecSizeBytes) {
        const char *mime;
        if (mMeta->findCString(kKeyMIMEType, &mime)) {
            if (mIsAudio) {
                mEstimatedCodecSizeBytes = kBaseStsdABoxSize +
                                           kBaseSmhdBoxSize;

                if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AMR_NB, mime)
                        || !strcasecmp(MEDIA_MIMETYPE_AUDIO_AMR_WB, mime)) {
                    mEstimatedCodecSizeBytes += kBaseDamrBoxSize;
                } else if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mime)) {
                    mEstimatedCodecSizeBytes += kBaseEsdsABoxSize;
                }
            } else {
                mEstimatedCodecSizeBytes = kBaseStsdVBoxSize +
                                           kBasePaspBoxSize +
                                           kBaseVmhdBoxSize;

                if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG4, mime)) {
                    mEstimatedCodecSizeBytes += kBaseEsdsVBoxSize;
                } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_H263, mime)) {
                    mEstimatedCodecSizeBytes += kBaseD263BoxSize;
                } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
                    mEstimatedCodecSizeBytes += kBaseAvccBoxSize;
                }
            }
        }
    }
    ...
    if (!mIsAudio) {
        mEstimatedTrackSizeBytes += kBaseCttsBoxSize + kBaseCttsBoxSize +
            (mCttsTableEntries->count() + 1) * kEntryCttsBoxSize +
            (mStssTableEntries->count() + 1) * kEntryStssBoxSize;
    }
}
...
void MPEG4Writer::Track::writeStszBox() {
    ...
    mOwner->writeInt32(0);  // version=0, flags=0
    if (mSamplesHaveSameSize && mStszTableEntries->count() > 0) {
        uint32_t sampleSize;
        CHECK(mStszTableEntries->get(sampleSize, 0));
        mOwner->write(&sampleSize, sizeof(sampleSize));
